...
def main(args):
    ...
    if args.model_config_name_or_path is None and args.model_config is None:
        model = UNet2DModel(
            sample_size=(args.resolution[0]//4, args.resolution[1]//4),
            in_channels=vae_config['latent_channels']+8 if args.upsample else vae_config['latent_channels']+5,
        ...
        model = UNet2DModel(**args.model_config)
    else:
    ...
    # Initialize the contional encoder
    if args.upsample:
        condition_encoder = SparseRangeImageEncoder2()
        condition_encoder.to(accelerator.device)
    else:
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # encode condition
            if args.upsample:
                condition = batch["down"]
                condition = condition_encoder(condition)
            else:
                masked_image = batch["masked_image"]
                masked_image = vae.encode(masked_image.to(torch.float32 if args.mixed_precision == "no" else torch.float16)).latent_dist.sample()
                masked_image = masked_image * vae.config.scaling_factor
                mask = torch.nn.functional.interpolate(batch["inpainting_mask"],
                                                     size=masked_image.shape[-2:])
                condition = torch.cat([masked_image, mask], dim=1)
            ...
            noisy_images = torch.cat([noisy_images, condition], dim=1)
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                pipeline = LDMUpscalePipelineRange(
                    unet=unet,
                ...
                #     save_generated(image, os.path.join(args.output_dir, f"epoch-{epoch}", f"generated-{epoch}-{img_id}"))
