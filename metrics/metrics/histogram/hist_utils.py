...
def load_pts_to_range_images(sample_folder):
    full_list = glob.glob(f'{sample_folder}/*.bin')
...
def get_histogram_for_range(range_image, nus=False):
    point_cloud = range_image_to_point_cloud_fast(range_image) if not nus else range_image_to_point_cloud_fast_nuscenes(range_image)
...
def array_to_histograms(samples, nus=False):
    result = []
    ...
    for sample in samples:
        ...
        with np.errstate(over='raise'):
            try:
                ...
                continue
        '''
        hist = get_histogram_for_range(sample, nus)
        result.append(hist)
...
def load_nuscenes_laserscan(file):
    ...
    result.set_points(points[:, 0:3], remissions=points[:, 3])
    result.do_range_projection()

    range_image = result.proj_range
    range_image = np.where(range_image < 0, 0, range_image) + 0.0001
    range_image = ((np.log2(range_image+1)) / 6.5)
