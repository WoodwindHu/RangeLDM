...
from .lpips import spatial_average
...
class Backbone(nn.Module):
    ...
    def load_from_pretrained(self, name="darknet53"):
        ckpt = get_ckpt_path(name, "sgm/modules/autoencoding/lpips/loss")
        self.load_state_dict(
            torch.load(ckpt, map_location=torch.device("cpu")), strict=False
        )
        print("loaded pretrained darknet53 loss from {}".format(ckpt))
    ...
    def forward(self, input, target):
        # filter input
        input = input[:, self.input_idxs]
        target = target[:, self.input_idxs]

        in0_input = self.relu1(self.bn1(self.conv1(input)))
        in1_input = self.relu1(self.bn1(self.conv1(target)))
        feats0, feats1, diffs = {0: in0_input}, {0: in1_input}, {0: (in0_input - in1_input) ** 2}
        encs = [self.enc1, self.enc2, self.enc3, self.enc4, self.enc5]
        for kk in range(len(encs)):
            feats0[kk+1], feats1[kk+1] = encs[kk](feats0[kk]), encs[kk](feats1[kk])
            diffs[kk+1] = (feats0[kk+1] - feats1[kk+1]) ** 2
        
        res = [
                torch.mean(diffs[kk], dim=[1, 2, 3], keepdim=True)
                for kk in range(len(diffs))
            ]
        val = res[0]
